...
public class AnnualEventServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            AnnuEvents annualEvent = new AnnuEvents(event_name, date, venue, remark, "", "", university);

            AnnualEventFunctions Functions = new AnnualEventFunctions();
            if (isEdit.equals(FormStatus.NEW)) {
                Functions.saveAnnualEvent(annualEvent);
            } else {
                annualEvent.setIndexNo(isEdit);
                Functions.editAnnualEvent(annualEvent);

            }

            response.setContentType("text/plain");
            response.setCharacterEncoding("UTF-8");
            response.getWriter().write("Event Creation Successfuly");
